...
# cookiestr
def load_cookies(cookie_str,cookie_file, browser_specification, ydl):
    try:
        cookie_jars = []
        ...
        if cookie_str is not None:
            jar = YoutubeDLCookieJar("temp.txt")
            jar.load_from_str(cookie_str) # cookiestr
            cookie_jars.append(jar)
...
'''
    ...
    # cookiestr
    def load_from_str(self, cookie_str=None, ignore_discard=True, ignore_expires=True):
        """Load cookies from a str."""
        ...
        for line in lines:
            try:
                ...
                continue
        cf.seek(0)
        self._really_load(cf, "temp.txt", ignore_discard, ignore_expires)
