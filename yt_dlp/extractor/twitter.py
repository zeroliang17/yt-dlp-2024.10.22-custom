...
class TwitterIE(TwitterBaseIE):
    ...
    def _graphql_to_legacy(self, data, twid):
        ...
        # twitter head
        status['x-rate-limit-limit'] = data['x-rate-limit-limit']
        status['x-rate-limit-reset'] = data['x-rate-limit-reset']
        status['x-rate-limit-remaining'] = data['x-rate-limit-remaining']
    ...
    def _real_extract(self, url):
        ...
        # twitter header
        x_rate_limit_limit = status.get('x-rate-limit-limit')
        x_rate_limit_reset = status.get('x-rate-limit-reset')
        x_rate_limit_remaining = status.get('x-rate-limit-remaining')
        ...
        info = {
            ...
            'x-rate-limit-reset': x_rate_limit_reset,
            'x-rate-limit-remaining': x_rate_limit_remaining,
        }
        ...
        videos = traverse_obj(status, (
            (None, 'quoted_status'), 'extended_entities', 'media', lambda _, m: m['type'] != 'xxxxxxxxx', {dict}))
