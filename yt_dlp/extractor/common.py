...
class InfoExtractor:
        ...
        def download_content(self, url_or_request, video_id, note=note, errnote=errnote, transform_source=None,
            ...
            if self.get_param('load_pages'):
                ...
                try:
                    ...
                    return parse(self, content, video_id, transform_source=transform_source, fatal=fatal,
                                 errnote=errnote)
            kwargs = {
                'note': note,
                'errnote': errnote,
            ...
            # twitter header
            if str(res[1].url).startswith("https://x.com/"):
                target_headers = {key: value for key, value in res[1].headers._headers if
                                  key in ['x-rate-limit-reset', 'x-rate-limit-limit', 'x-rate-limit-remaining']}
                if target_headers:
                    res[0]['data']['x-rate-limit-reset'] = target_headers['x-rate-limit-reset']
                    res[0]['data']['x-rate-limit-limit'] = target_headers['x-rate-limit-limit']
                    res[0]['data']['x-rate-limit-remaining'] = target_headers['x-rate-limit-remaining']
    ...
    @staticmethod
    def playlist_result(entries, playlist_id=None, playlist_title=None, playlist_description=None, *, multi_video=False,
                        **kwargs):
        """Returns a playlist"""
        if playlist_id:
    ...
    def _json_ld(self, json_ld, video_id, fatal=True, expected_type=None):
        ...
        def extract_video_object(e):
            ...
            info.update({
                ...
                'uploader': author.get('name') if isinstance(author, dict) else author if isinstance(author,
                                                                                                     str) else None,
    ...
    def _extract_smil_formats_and_subtitles(self, smil_url, video_id, fatal=True, f4m_params=None,
    ...
    def _extract_ism_formats_and_subtitles(self, ism_url, video_id, ism_id=None, note=None, errnote=None, fatal=True,
    ...
    def _parse_html5_media_entries(self, base_url, webpage, video_id, m3u8_id=None, m3u8_entry_protocol='m3u8_native',
                                   mpd_id=None, preference=None, quality=None, _headers=None):
            ...
            if media_content:
                ...
                for track_tag in re.findall(r'<track[^>]+>', media_content):
                    ...
                    if not kind or kind in ('subtitles', 'captions'):
                        ...
                        if not src:
                            continue
                        lang = track_attributes.get('srclang') or track_attributes.get('lang') or track_attributes.get(
                            'label')
    ...
    def _cookies_passed(self):
        ...
        # cookieStr
        return self.get_param('cookiefile') is not None or self.get_param(
            'cookiesfrombrowser') is not None or self.get_param('cookieStr') is not None
    ...
    def _yes_playlist(self, playlist_id, video_id, smuggled_data=None, *, playlist_label='playlist',
                      video_label='video'):
